...
namespace demo
{
    class WoSaiPay
    {
        ...
        public static JObject activate(string vendor_sn,string vendor_key,string code)
        {
            string url = api_domain + "/terminal/activate";
            JObject Jparams = new JObject();

            Jparams.Add(new JProperty("code", code));
            Jparams.Add(new JProperty("type", "50"));
            Jparams.Add(new JProperty("device_id", "1"));
            Jparams.Add(new JProperty("app_id", ""));
